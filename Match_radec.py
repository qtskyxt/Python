...
#读取 HSC 数据，1ra 2dec 3-6g,r,i,z 7-10g,r,i,z err 0增加编码
HSC=np.loadtxt(pwd+'213641.csv',delimiter=',')
...
for i in range(S82.shape[0]):
    S82s[int(S82[i][0])].append(HSC[i])


#定义函数，判断两者是否在 1 角秒的范围内，返回 T or F
def dis(i,j):
    d2 = (HSC[i][0] - S82[j][0]) ** 2 + (HSC[i][1] - S82[j][1]) ** 2
...
#创建记录文件，记录匹配的总数和编号
match=open(pwd+'match.txt','w')

#根据HSC每一行的目标寻找匹配的Stripe82中的目标
for k in range(360):
    for i in range(HSCs[k].shape[0]):
        for j in range(S82s[k].shape[0]):
            n=0
            if dis(i,j):
                n=n+1
                match.write(str(j)+' ')
