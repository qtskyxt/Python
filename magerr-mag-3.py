#横轴为 error 纵轴为 star 个数，不同颜色代表不同的光度区间 重叠每个 gram
...
nerrl=[[]]*(m+1)
for i in range(m+1):
	nerrl[i]=round(nerr[i],4)
for i in range(n+1):
	nmag[i]=round(nmag[i],4)
nerrl=nerrl[0::3]
plt.xticks(xindex-0.5,nerrl)
plt.ylabel('N')
#plt.yscale('log')
...
t=np.array([0]*m)
for i in range(n):
	plt.bar(index, q[i], width=bar_width, bottom=t,label='i_mag='+str(nmag[i])+'~'+str(nmag[i+1]))
	t=t+np.array(q[i])
